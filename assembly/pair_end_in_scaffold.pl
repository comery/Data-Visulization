...
use Data::Dumper;
use lib "/home/xiongzj/bin/assemble/PE";
use SVG;
use FontSize;
...
sub get_the_map()
{
   ...
   foreach my $name (keys(%contig_compose))
   {
      ...
      my $svg=SVG->new(width=>($length*10)/(9*$bp_per_pix),height=>500);
      my $path=$svg->group(
      ...
      foreach my $key_1 (sort {$a<=>$b} keys %{$contig_compose{$name}})
      {
		 ...
		 if($c_len != 0){
         	$rect->rectangle(
         	   x=>$x_start+($c_s-$start)/$bp_per_pix,y=>450,
      ...
      {
	     next unless($key_2 >= $start && $hash_pair_info{$name}{20000}{$key_2} <= $end);
      	 my $len=($hash_pair_info{$name}{20000}{$key_2}-$key_2)/$bp_per_pix;
      	 my $min=$x_start+($key_2-$start)/$bp_per_pix;
         $path->path(
      ...
      {
	     next unless($key_2 >= $start && $hash_pair_info{$name}{10000}{$key_2} <= $end);
      ...
      {
	     next unless($key_2 >= $start && $hash_pair_info{$name}{5000}{$key_2} <= $end);
      ...
      {
	     next unless($key_2 >= $start && $hash_pair_info{$name}{2000}{$key_2} <= $end);
      ...
      {
         next unless($key_2 >= $start && $hash_pair_info{$name}{700}{$key_2} <= $end);
      ...
      {
	     next unless($key_2 >= $start && $hash_pair_info{$name}{300}{$key_2} <= $end);
      ...
      close OUT;
	  #system("/nas/GAG_01B/assembly/pig/list/distributing_svg_4.74/svg2xxx_release/svg2xxx -t png $name.svg");
   }
...
sub get_pair_info() #读取SOAP结果，得到pair-end关系的信息。
{
   ...
   while(<LIST>)
   {
      chomp;
	  if ($_ =~ /\.bam$/){
	  	open(SOAP, "/share/app/samtools-1.2/bin/samtools view $_ |") or die $!;
	  }
	  else{
      	open SOAP,"$_";
	  }
	  while(<SOAP>)
      {
         ...
         if(defined($pos{$tmp_1[2]}))
         {
			...
			if($tmp_2[0] ne $tmp_1[0]){
				@tmp_1 = @tmp_2;
				goto START;
			}
			
			#my $distance=abs($tmp_1[8]-$tmp_2[8]);
			my $distance = abs($tmp_1[8]);
			die "$_\n$line\n" if(abs($tmp_2[8]) != $distance);
            #########################################################################
            if($distance>=180&&$distance<420)
            {
               $flag=0;
               ...
               {
                  $flag=$flag||(defined($pair_info{$tmp_1[2]}{300}{$tmp_2[3]+$ii}))
               }
               if(!$flag)
               {
                  if($tmp_1[3]>=$tmp_2[3])
                     {$pair_info{$tmp_1[2]}{300}{$tmp_2[3]}=$tmp_1[3];}
                  else
                     {$pair_info{$tmp_1[2]}{300}{$tmp_1[3]}=$tmp_2[3];}
               }
            }
            #########################################################################
            elsif($distance>=420&&$distance<=980)
            {
               ...
               for(my $ii=-$allow_gap;$ii<=$allow_gap;$ii++)
               {
                  $flag=$flag||(defined($pair_info{$tmp_1[2]}{700}{$tmp_2[3]+$ii}))
               }
               if(!$flag)
               {
                  if($tmp_1[3]>=$tmp_2[3])
                     {$pair_info{$tmp_1[2]}{700}{$tmp_2[3]}=$tmp_1[3];}
                  else
                     {$pair_info{$tmp_1[2]}{700}{$tmp_1[3]}=$tmp_2[3];}
               }
            }
            #########################################################################
            elsif($distance>=1600&&$distance<=2400)
            {
               ...
               for(my $ii=-$allow_gap;$ii<=$allow_gap;$ii++)
               {
                  $flag=$flag||(defined($pair_info{$tmp_1[2]}{2000}{$tmp_2[3]+$ii}))
               }
               if(!$flag)
               {
                  if($tmp_1[3]>=$tmp_2[3])
                     {$pair_info{$tmp_1[2]}{2000}{$tmp_2[3]}=$tmp_1[3];}
                  else
                     {$pair_info{$tmp_1[2]}{2000}{$tmp_1[3]}=$tmp_2[3];}
               }
            }
            #########################################################################
            elsif($distance>=4000&&$distance<=6000)
            {
               ...
               for(my $ii=-$allow_gap;$ii<=$allow_gap;$ii++)
               {
                  $flag=$flag||(defined($pair_info{$tmp_1[2]}{5000}{$tmp_2[3]+$ii}))
               }
               if(!$flag)
               {
                  if($tmp_1[3]>=$tmp_2[3])
                     {$pair_info{$tmp_1[2]}{5000}{$tmp_2[3]}=$tmp_1[3];}
                  else
                     {$pair_info{$tmp_1[2]}{5000}{$tmp_1[3]}=$tmp_2[3];}
               }
            }
            #########################################################################
            elsif($distance>=8000&&$distance<=12000)
            {
               ...
               for(my $ii=-$allow_gap;$ii<=$allow_gap;$ii++)
               {
                  $flag=$flag||(defined($pair_info{$tmp_1[2]}{10000}{$tmp_2[3]+$ii}))
               }
               if(!$flag)
               {
                  if($tmp_1[3]>=$tmp_2[3])
                     {$pair_info{$tmp_1[2]}{10000}{$tmp_2[3]}=$tmp_1[3];}
                  else
                     {$pair_info{$tmp_1[2]}{10000}{$tmp_1[3]}=$tmp_2[3];}
               }
            }
            #########################################################################
            elsif($distance>=16000&&$distance<=24000)
